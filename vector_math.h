...
void v3f_cross_v3fo(const float* x, const float* y, float* result) {
  result[0] = x[1]*y[2] - x[2]*y[1];
  result[1] = x[2]*y[0] - x[0]*y[2];
  result[2] = x[0]*y[1] - x[1]*y[0];
}

float magnitude_v3f(const float* x) {
  return sqrt(x[0]*x[0] + x[1]*x[1] + x[2]*x[2]);
}

void scale_v3f(float scalar, float* x) {
  x[0] *= scalar;
  x[1] *= scalar;
  x[2] *= scalar;
}
...
void difference_v3fo(const float* x, const float* y, float* result) {
  result[0] = x[0] - y[0];
  result[1] = x[1] - y[1];
  result[2] = x[2] - y[2];
}

void normalize_v3f(float* x) {
