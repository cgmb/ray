...
inline void v3f_cross_v3fo(const float* x, const float* y, float* result) {
...
inline void translate_v3fo(float x, float y, float z, const float* v, float* result) {
  result[0] = v[0] + x;
  result[1] = v[1] + y;
  result[2] = v[2] + z;
}

inline void difference_v3fo(const float* x, const float* y, float* result) {
  result[0] = x[0] - y[0];
  result[1] = x[1] - y[1];
  result[2] = x[2] - y[2];
}
...
inline void m3f_copy_m3fo(const float* m, float* result) {
  memcpy(result, m, 9 * sizeof(float));
}

inline void m3f_mul_m3fo(const float* m, const float* n, float* result) {
...
inline void m3f_mul_v3fo(const float* m, const float* v, float* result) {
  result[0] = m[0]*v[0] + m[1]*v[1] + m[2]*v[2];
  result[1] = m[3]*v[0] + m[4]*v[1] + m[5]*v[2];
  result[2] = m[6]*v[0] + m[7]*v[1] + m[8]*v[2];
}

inline void m3f_mul_av3fo(float* m, const float* input, size_t count, float* result) {
  size_t i;
  for(i = 0; i < count; i += 3) {
...
inline void rotatex_v3fo(float rx, const float* v, float* result) {
  float rm[9];
  m3f_fill_rotx_m3fo(rx, rm);
  m3f_mul_m3fo(rm, v, result);
}

inline void rotatey_v3fo(float ry, const float* v, float* result) {
  float rm[16];
  m3f_fill_roty_m3fo(ry, rm);
  m3f_mul_m3fo(rm, v, result);
}

inline void m4f_copy_m4fo(const float* m, float* result) {
  memcpy(result, m, 16 * sizeof(float));
}

inline void m4f_mul_m4fo(const float* m, const float* n, float* result) {
...
inline void m4f_mul_m4f(const float* in, float* in_out) {
  float temp[16];
  m4f_mul_m4fo(in, in_out, temp);
  m4f_copy_m4fo(temp, in_out);
}

inline void m4f_mul_v4fo(const float* m, const float* v, float* result) {
...
inline void rotatex_v4fo(float rx, const float* v, float* result) {
  float rm[16];
  m4f_fill_rotx_m4fo(rx, rm);
  m4f_mul_m4fo(rm, v, result);
}

inline void rotatey_v4fo(float ry, const float* v, float* result) {
  float rm[16];
